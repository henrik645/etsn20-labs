...
class NextDateFullStatementCoverageTest {
	...
	void testInvalidDate()
	{
		...
		assertEquals("Invalid Input Date", nextDate.run(0, 15, 2001));
		// these could be useful later when testing all predicates:
//		assertEquals("Invalid Input Date", nextDate.run(1, 0, 2001));
//		assertEquals("Invalid Input Date", nextDate.run(1, 1, 0));
//		assertEquals("Invalid Input Date", nextDate.run(1, 1, 2022));
//		assertEquals("Invalid Input Date", nextDate.run(1, 1, 1765));
	}
